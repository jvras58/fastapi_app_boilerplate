...
class CustomLog:
    ...
    def _log(self, title: str) -> None:
        ...
        _console.print()
        _console.rule(
            f'[{self.color}] {_logger_id} > {title} | {_status_code} | {_time}[/] ',  # noqa: E501
            style=f'{self.color}',
        )
        ...
        _console.print()
        _console.rule(
            f'[{self.color}]{self.execute_time} miliseconds[/]',
            style=f'{self.color}',
        )
        _console.print()
        _console.print()
...
async def log_middleware(request: Request, call_next: Callable) -> Response:
    ...
    if get_settings().LOG_LEVEL == 'DEBUG':
        try:
            ...
            retorno = await call_next(request)
    else:
        retorno = await call_next(request)
