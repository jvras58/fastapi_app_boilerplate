...
from communs.exceptions import (
    IncorrectCredentialError as IncorrectCredentialException,
)
from communs.security import (
    create_access_token,
    extract_username,
    verify_password,
)
...
def execute_user_login(db_session: Session, # type: ignore  # noqa: PGH003
...
async def get_current_user(db_session: Session, # type: ignore  # noqa: PGH003
    token: OAuth2Token) -> User:
    """Get current user."""
    ...
    db_user = user_controller.get_user_by_username(db_session,
    token_data.username)
